    ...
    for volume in volumes:
        for slide in volumes:
            ...
            for _ in range(num_of_folders):
                folder_kind = fake.random_element(elements=('audio', 'video', 'image', 'office', 'text'))

                folder_path = original_folder.joinpath(volume).joinpath(SLIDE_DEFAULT_NAME).joinpath(slide).joinpath(folder_kind)
                folder_path.mkdir(parents=True, exist_ok=True)
                print(f"{folder_path} created")

                expected_folder.joinpath(volume).joinpath(SLIDE_DEFAULT_NAME).joinpath(slide).joinpath(folder_kind).mkdir(parents=True, exist_ok=True)

                folder_path_ = expected_folder.joinpath(slide).joinpath(SLIDE_DEFAULT_NAME).joinpath(slide).joinpath(folder_kind)
                folder_path_.mkdir(parents=True, exist_ok=True)

                num_of_files = fake.random_int(min=1, max=10)

                for _ in range(num_of_files):  
                    file_path = folder_path.joinpath(fake.file_name(category=folder_kind))
                    num_of_bytes = fake.random_int(min=1, max=10)

                    file_path.write_text(fake.text(max_nb_chars=num_of_bytes*1024))

                    sha256_hash = hashlib.sha256()
                    with open(file_path, "rb") as f, open(f"{file_path}.sha256", "w") as cf:
